...
def learning_rate_with_decay(
  ...
  def learning_rate_fn(global_step):
    ...
    if batch_size * hvd.size() <= 2048:
      ...
      def f1(global_step):
        return 0.1 + (initial_learning_rate - 0.1) * \
                tf.cast(global_step, dtype=tf.float32) / 20 /batches_per_epoch
      def f2(global_step):
        return tf.train.piecewise_constant(global_step, boundaries, vals)
      return tf.cond( tf.less(global_step, 20 * int(batches_per_epoch)), lambda: f1(global_step), lambda: f2(global_step))
...
class ResnetArgParser(argparse.ArgumentParser):
  ...
  def __init__(self, resnet_size_choices=None):
    ...
    self.add_argument(
        '--resnet_size', type=int, default=20,
        choices=resnet_size_choices,
        help='The size of the ResNet model to use.')

    self.add_argument(
        '--train_epochs', type=int, default=150,
